# Travel Visa — AIIB Interactive Dashboard

Single-file, offline **Excel-backed** dashboard branded in the AIIB
(Asian Infrastructure Investment Bank) visual style — AIIB Blue `#003A70` with
AIIB Gold `#EAAA00` accents. No backend, no database, no login. Data is fed via
a standardized Excel (`.xlsx`) file and processed entirely in the browser.

## Files

```
docs/
  index.html   ← the complete dashboard (open in any browser)
  .nojekyll    ← disables Jekyll (fixes the SCSS build error on GitHub Pages)
.github/
  workflows/
    deploy.yml ← deploys docs/ to GitHub Pages (static, no build step)
```

Excel is the database — the dashboard only reads the file you load and
(optionally) keeps a copy in your browser's localStorage for convenience.
...
- AIIB brand colors: Blue `#003A70`, Gold `#EAAA00`. Fonts use Google Fonts
  "Source Sans Pro" (an AIIB-friendly sans). If offline, the dashboard falls
  back to the system sans-serif — layout and colors are unaffected.
